...
function processAnyOfDiscriminatedUnion(schema, schemaName, allSchemas) {
    ...
    console.error(
        `  ✓ ${schemaName}: ${enumCases.length} cases (${casesWithValues} with values, ${casesWithoutValues} without) [anyOf pattern]`,
    );
...
function processAllOfDiscriminatedUnion(schema, schemaName, allSchemas) {
    ...
    console.error(
        `  ✓ ${schemaName}: ${enumCases.length} cases (${casesWithValues} with values, ${casesWithoutValues} without)`,
    );
...
try {
    console.error('📝 Post-processing OpenAPI spec...');
    ...
    for (const [name, schema] of Object.entries(schemas)) {
        ...
        if (
            processAllOfDiscriminatedUnion(schema, name, schemas) ||
