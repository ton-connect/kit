...
import { TonConnectUI } from '@ton/appkit-ui';
import type {
    ITonConnect,
    RequiredFeatures,
    ActionConfiguration,
...
export interface TonConnectUIProviderPropsWithConnector {
    /**
     * TonConnect instance. Can be helpful if you use custom ITonConnect implementation, or use both of @tonconnect/sdk and @ton/appkit-ui in your app.
     */
 ...
 * @constructor
 */
const TonConnectUIProvider: FunctionComponent<TonConnectUIProviderProps> = ({ children, ...options }) => {
    if (isClientSide() && !tonConnectUI) {
        tonConnectUI = new TonConnectUI(options);
    }

    return <TonConnectUIContext.Provider value={tonConnectUI}>{children}</TonConnectUIContext.Provider>;
};
