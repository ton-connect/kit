...
import { isInTelegramBrowser, isInTMA, isTmaPlatform, sendOpenTelegramLink } from 'src/app/utils/tma-api';
import { isBrowser, isOS, openDeeplinkWithFallback, openLink, openLinkBlank, toDeeplink } from 'src/app/utils/web-api';
import { decodeTelegramUrlParameters, encodeTelegramUrlParameters, isTelegramUrl } from '@ton/appkit';
...
export function enrichUniversalLink(
    ...
    const previousStartApp = decodeTelegramUrlParameters(directLinkUrl.searchParams.get('startapp') ?? '');
    ...
    // returns a link like: /start?startapp=tonconnect-v__2-id__{sessionId}-trace--5Fid__{traceId}
...
function addQueryParametersIfNotPresented(url: string, params: Record<string, string | null | undefined>): string {
    const parsed = new URL(url);
