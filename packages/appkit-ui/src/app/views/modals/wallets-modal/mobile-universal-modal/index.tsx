...
import { H1Styled, H2Styled, StyledLeftActionButton, TelegramButtonStyled, TGImageStyled } from './style';
...
export const MobileUniversalModal: Component<MobileUniversalModalProps> = (props) => {
    ...
    const atWalletSupportFeatures = createMemo(
        () =>
            props.walletsList.find((wallet) => wallet.appName === AT_WALLET_APP_NAME)?.isSupportRequiredFeatures ??
            false,
        null,
    );
    ...
    setLastVisibleWalletsInfo({
        ...
        wallets: atWalletSupportFeatures()
            ? [props.walletsList.find((wallet) => wallet.appName === AT_WALLET_APP_NAME)!, ...visibleWallets()]
            : visibleWallets(),
    });
