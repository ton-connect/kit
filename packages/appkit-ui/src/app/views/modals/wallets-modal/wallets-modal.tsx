...
import type { ConnectAdditionalRequest, Wallet, WalletInfoRemote, WalletInfo } from '@ton/appkit';
import {
...
import type { ChooseSupportedFeatureWalletsModal } from 'src/models/wallets-modal';
...
export const WalletsModal: Component = () => {
    ...
    const [selectedWalletError, setSelectedWalletError] = createSignal<'missing-features' | 'not-supported' | null>(
        null,
    );
    ...
    const walletsList = createMemo<UIWalletInfo[] | null>(() => {
        ...
        let walletsList = applyWalletsListConfiguration(fetchedWalletsList(), appState.walletsListConfiguration);
        ...
        const preferredWallet = walletsList.find((item) => eqWalletName(item, preferredWalletAppName));
        ...
        if (atWallet) {
            walletsList = [atWallet].concat(walletsList.filter((item) => !eqWalletName(item, AT_WALLET_APP_NAME)));
        }
                    ...
                    <Switch>
                        <Match when={walletsModalIsWalletNotSupportFeature()}>
                            <DesktopFeatureNotSupportModal
                                ...
                                walletsModalState={walletsModalState() as ChooseSupportedFeatureWalletsModal}
                                onClose={() => onClose('action-cancelled')}
                            />
                        </Match>
                        ...
                        <Match when={selectedTab() === 'universal'}>
                            <Dynamic
                                component={isMobile() ? MobileUniversalModal : DesktopUniversalModal}
                        ...
                        <Match when={selectedTab() === 'all-wallets'}>
                            <AllWalletsListModal
                                walletsList={walletsList()!}
                                featureCheckMode={tonConnectUI?.walletsRequiredFeatures ? 'strict' : 'soft'}
