...
import { isWalletConnectInitialized } from '@ton/appkit';
...
export const AllWalletsListModal: Component<DesktopSelectWalletModalProps> = (props) => {
    ...
    const supportedWallets = (): UIWalletInfo[] => walletsList().filter((wallet) => wallet.isSupportRequiredFeatures);
            ...
            <ScrollContainer maxHeight={maxHeight()}>
                <WalletsUl>
                    <For each={supportedWallets()}>
                        {(wallet) => (
                            <li>
                                <WalletLabeledItemStyled wallet={wallet} onClick={() => props.onSelect(wallet)} />
                            </li>
                        )}
                    </For>
                ...
                <Show when={unsupportedWallets().length > 0 && props.featureCheckMode !== 'hide'}>
                    <WalletsNotSupportedNotifier>
                        <WalletsNotSupportedNotifierText translationKey="walletModal.allWallets.walletsBelowNotSupported">
                            The wallets below don’t support all features of the connected service. You can use your
                            recovery phrase in one of the supported wallets above.
                        </WalletsNotSupportedNotifierText>
