...
import { isTelegramUrl } from '@ton/appkit';
...
import { ActionModalStyled, ButtonStyled, H1Styled, LoaderButtonStyled, LoaderIconStyled, TextStyled } from './style';
...
export const ActionModal: Component<ActionModalProps> = (props) => {
    ...
    createEffect(() => {
        ...
        setSent(
            !!currentAction &&
                (('sent' in currentAction && currentAction.sent) || currentAction.name === 'transaction-sent'),
        );
        setSigned(
            !!currentAction &&
                (('signed' in currentAction && currentAction.signed) || currentAction.name === 'data-signed'),
        );
        setCanceled(
            !!currentAction &&
                (currentAction.name === 'transaction-canceled' || currentAction.name === 'sign-data-canceled'),
        );
    });
    ...
    const onOpenWallet = (): void => {
        ...
        const returnStrategy =
            'returnStrategy' in currentAction ? currentAction.returnStrategy : appState.returnStrategy;
    ...
    return (
        <ActionModalStyled {...dataAttrs}>
            {props.icon}
            <H1Styled translationKey={props.headerTranslationKey} translationValues={props.headerTranslationValues} />
            <TextStyled translationKey={props.textTranslationKey} translationValues={props.textTranslationValues} />
            <Show
                when={
                    !sent() &&
                    !signed() &&
                    !canceled() &&
                    ((props.showButton === 'open-wallet' && universalLink) || props.showButton !== 'open-wallet')
                }
            >
