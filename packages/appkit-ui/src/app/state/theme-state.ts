...
import type { BorderRadius, PartialColorsSet } from 'src/models';
...
export function setColors(colorsSet: Partial<Record<THEME, PartialColorsSet>>): void {
    ...
    setThemeState((state) => ({
        colors: mergeOptions(themeCustomColors[state.theme as THEME], themeColorsMappingDefault[state.theme as THEME]),
    }));
}
