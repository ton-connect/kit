...
import type { Action, ActionName } from 'src/app/state/modals-state';
...
export function useOpenedNotifications(config?: UseOpenedNotificationsConfig): UseOpenedNotifications {
    ...
    createEffect(
        on(action, (action: Action | null): void => {
            ...
            const isConfirmTransactionAction =
                latestAction()?.name === 'confirm-transaction' && action.name === 'confirm-transaction';
            ...
            const timeoutId = setTimeout(() => {
                setOpenedNotifications((openedNotifications) => openedNotifications.filter((n) => n !== notification));
