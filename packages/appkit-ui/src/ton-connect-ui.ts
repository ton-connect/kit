...
import { BrowserEventDispatcher, UUIDv7 } from '@ton/appkit';
import type {
    ITonConnect,
...
import type { TonConnectUiCreateOptions } from 'src/models/ton-connect-ui-create-options';
import { PreferredWalletStorage, WalletInfoStorage } from 'src/storage';
import { createMacrotaskAsync, getSystemTheme, preloadImages, subscribeToThemeChange } from 'src/app/utils/web-api';
import type { TonConnectUiOptions } from 'src/models/ton-connect-ui-options';
...
import type { ActionConfiguration, StrictActionConfiguration } from 'src/models/action-configuration';
import type { ConnectedWallet, WalletInfoWithOpenMethod } from 'src/models/connected-wallet';
...
import type { WalletsModal, WalletsModalCloseReason, WalletsModalState } from 'src/models/wallets-modal';
import { isInTMA, sendExpand } from 'src/app/utils/tma-api';
import { redirectToTelegram, redirectToWallet, enrichUniversalLink } from 'src/app/utils/url-strategy-helpers';
import { SingleWalletModalManager } from 'src/managers/single-wallet-modal-manager';
import type { SingleWalletModal, SingleWalletModalState } from 'src/models/single-wallet-modal';
...
import type { ReturnStrategy } from './models';
...
export class TonConnectUI {
    ...
    private connectRequestParametersCallback?: (parameters: ConnectAdditionalRequest | undefined) => void;
    ...
    constructor(options?: TonConnectUiCreateOptions) {
        ...
        this.modal = new WalletsModalManager({
            ...
            setConnectRequestParametersCallback: (callback: (parameters?: ConnectAdditionalRequest) => void) => {
     ...
     * @experimental
     */
    public onSingleWalletModalStateChange(onChange: (state: SingleWalletModalState) => void): () => void {
    ...
    public async signData(
        ...
        const { notifications, modals, returnStrategy, twaReturnUrl } = this.getModalsAndNotificationsConfiguration();
    ...
    private redirectAfterRequestSent({
        ...
        if (
            ...
            (this.walletInfo.openMethod === 'universal-link' || this.walletInfo.openMethod === 'custom-deeplink')
        ) {
     ...
     * @internal
     */
    private async waitForWalletConnection(options: WaitWalletConnectionOptions): Promise<ConnectedWallet> {
     ...
     * @internal
     */
    private async waitForSignData(options: WaitSignDataOptions, onRequestSent?: () => void): Promise<SignDataResponse> {
        return new Promise((resolve, reject) => {
    ...
    private async updateWalletInfo(wallet: Wallet): Promise<void> {
        ...
        this.walletInfo =
            (await this.walletsList).find((walletInfo) => eqWalletName(walletInfo, wallet.device.appName)) || null;
    }
    ...
    private getModalsAndNotificationsConfiguration(options?: ActionConfiguration): StrictActionConfiguration {
        const allActions: StrictActionConfiguration['notifications'] = ['before', 'success', 'error'];

        let notifications: StrictActionConfiguration['notifications'] = allActions;
        if (this.actionsConfiguration?.notifications && this.actionsConfiguration?.notifications !== 'all') {
        ...
        let skipRedirectToWallet =
            options?.skipRedirectToWallet || this.actionsConfiguration?.skipRedirectToWallet || 'ios';
