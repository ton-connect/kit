---
name: ton-swap
description: Swap or trade tokens on the TON blockchain via DEX. Use when the user wants to swap, trade, exchange, convert, buy, or sell tokens like GRAM (ex. TON), USDT, jUSDC, or any jetton. Covers phrases like "swap GRAM (ex. TON) for USDT", "trade tokens", "buy USDT", "exchange jettons".
user-invocable: true
...
2. Call `get_swap_quote` with `fromToken`, `toToken`, and `amount`
   - Use `"TON"` for native GRAM (ex. TON), or the jetton master contract address for tokens
   - Amount is human-readable (e.g., `"1.5"` = 1.5 GRAM (ex. TON))
3. Call `emulate_transaction` with the quote's `transaction.messages` to dry-run the swap — verify the expected money flow (GRAM (ex. TON)/jetton balance changes) before sending
