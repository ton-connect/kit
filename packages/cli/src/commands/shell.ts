...
import { balanceCommand, jettonBalanceCommand, balanceByAddressCommand } from './balance.js';
import { transactionsCommand, transactionStatusCommand } from './transactions.js';
import {
    jettonsCommand,
    knownJettonsCommand,
    jettonsByAddressCommand,
    jettonInfoCommand,
    jettonWalletAddressCommand,
} from './jettons.js';
import { dnsResolveCommand, dnsReverseCommand } from './dns.js';
import { nftListCommand, nftListByAddressCommand, nftGetCommand, nftSendCommand } from './nft.js';
...
function printShellHelp(): void {
    ...
    process.stderr.write(dim('    balance address <addr>           Get TON balance by address\n'));
    ...
    process.stderr.write(dim('    jettons by-address <addr>        List tokens by address\n'));
    process.stderr.write(dim('    jettons info <jetton>            Get jetton metadata\n'));
    process.stderr.write(dim('    jettons wallet-address <j> <o>   Get jetton wallet address\n'));
    process.stderr.write(dim('    transactions [--limit N]         Transaction history\n'));
    process.stderr.write(dim('    transactions status <hash>       Transaction status by normalized hash\n'));
    ...
    process.stderr.write(dim('    nft list-by-address <addr>       List NFTs by address\n'));
...
async function executeShellCommand(service: CliWalletService, input: string): Promise<boolean> {
    ...
    switch (cmd) {
        ...
        case 'balance':
            if (args[0] === 'address' && args[1]) {
                await balanceByAddressCommand(service, args[1], false);
            } else if (args[0] === 'jetton' && args[1]) {
                ...
                shellHint(['balance address <wallet-address>']);
            } else {
        ...
        case 'jettons':
            if (args[0] === 'known') {
            ...
            } else if (args[0] === 'by-address' && args[1]) {
                ...
                await jettonsByAddressCommand(service, args[1], limit, offset, false);
            } else if (args[0] === 'info' && args[1]) {
                await jettonInfoCommand(service, args[1], false);
            } else if (args[0] === 'wallet-address' && args[1] && args[2]) {
                await jettonWalletAddressCommand(service, args[1], args[2], false);
            } else if (args[0] === 'by-address') {
                shellHint(['jettons by-address <owner-address> [--limit N] [--offset N]']);
            } else if (args[0] === 'info') {
                shellHint(['jettons info <jetton-address>']);
            } else if (args[0] === 'wallet-address') {
                shellHint(['jettons wallet-address <jetton-address> <owner-address>']);
            } else {
        ...
        case 'transactions': {
            if (args[0] === 'status' && args[1]) {
                await transactionStatusCommand(service, args[1], false);
                break;
            }
        ...
        case 'send':
            if (args[0] === 'ton' && args[1] && args[2]) {
                ...
                if (messagesJson) {
                    ...
                    shellHint(['send raw --messages \'{"messages":[{"address":"...","amount":"..."}]}\'']);
                }
            } else if (args[0] === 'ton') {
        ...
        case 'nft':
            if (args[0] === 'list') {
            ...
            } else if (args[0] === 'list-by-address' && args[1]) {
                const limitIdx = args.indexOf('--limit');
                const limit = limitIdx !== -1 && args[limitIdx + 1] ? parseInt(args[limitIdx + 1], 10) : 20;
                const offsetIdx = args.indexOf('--offset');
                const offset = offsetIdx !== -1 && args[offsetIdx + 1] ? parseInt(args[offsetIdx + 1], 10) : 0;
                await nftListByAddressCommand(service, args[1], limit, offset, false);
            ...
            } else if (args[0] === 'list-by-address') {
                shellHint(['nft list-by-address <owner-address> [--limit N] [--offset N]']);
            } else if (args[0] === 'get') {
                ...
                shellHint([
                    ...
                    'nft list-by-address <a>   List NFTs by address',
