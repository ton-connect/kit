...
export async function jettonsByAddressCommand(
    ...
    try {
        const jettons = await withSpinner('Fetching tokens by address...', () =>
            service.getJettonsByAddress(address, limit, offset),
        );

        const data = { success: true, address, jettons, count: jettons.length, limit, offset };
        ...
        let human = header(`Tokens (${jettons.length}) for ${address}`);
        if (jettons.length > 0) {
            const rows = jettons.map((j) => [j.symbol || '???', j.balance, dim(j.address)]);
            human += tableRows([[dim('Symbol'), dim('Balance'), dim('Address')], ...rows]);
        } else {
...
export async function jettonInfoCommand(
    service: CliWalletService,
    jettonAddress: string,
    jsonMode: boolean,
): Promise<void> {
    try {
        const jetton = await withSpinner('Fetching jetton info...', () => service.getJettonInfo(jettonAddress));
        if (!jetton) {
            printError(jsonMode, 'Jetton not found');
            return;
        }

        const data = { success: true, jetton };
        const human =
            header('Jetton Info') +
            tableRows([
                [dim('Address'), jetton.address],
                [dim('Name'), jetton.name || '-'],
                [dim('Symbol'), jetton.symbol || '-'],
                [dim('Decimals'), jetton.decimals !== undefined ? String(jetton.decimals) : '-'],
                [dim('Description'), jetton.description || '-'],
                [dim('Image'), jetton.image || '-'],
                [dim('URI'), jetton.uri || '-'],
            ]);
...
export async function jettonWalletAddressCommand(
    service: CliWalletService,
    jettonAddress: string,
    ownerAddress: string,
    jsonMode: boolean,
    ...
    try {
        const jettonWalletAddress = await withSpinner('Fetching jetton wallet address...', () =>
            service.getJettonWalletAddress(jettonAddress, ownerAddress),
        );
        ...
        const human =
            header('Jetton Wallet Address') +
            tableRows([
                [dim('Jetton'), jettonAddress],
                [dim('Owner'), ownerAddress],
                [dim('Jetton Wallet'), jettonWalletAddress],
            ]);
