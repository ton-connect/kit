...
export async function balanceByAddressCommand(
    service: CliWalletService,
    address: string,
    jsonMode: boolean,
): Promise<void> {
    try {
        const result = await withSpinner('Fetching balance by address...', () => service.getBalanceByAddress(address));
        ...
        const human =
            ...
            keyValue([
                ['Address', result.address],
                ['TON', `${result.balanceTon} TON (${result.balanceNano} nanoTON)`],
            ]);
