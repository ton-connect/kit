...
export async function nftListByAddressCommand(
    service: CliWalletService,
    address: string,
    limit: number,
    offset: number,
    ...
    try {
        ...
        const data = { success: true, address, nfts: mapped, count: mapped.length, limit, offset };
        ...
        let human = header(`NFTs (${nfts.length}) for ${address}`);
        if (nfts.length > 0) {
            const rows = nfts.map((nft) => [nft.name || 'Unnamed', nft.collection?.name || '-', dim(nft.address)]);
            human += tableRows([[dim('Name'), dim('Collection'), dim('Address')], ...rows]);
        } else {
