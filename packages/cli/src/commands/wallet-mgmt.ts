...
export async function walletImportCommand(
    ...
    if (privateKey) {
        try {
            normalizedPrivateKey = `0x${parsePrivateKeyInput(privateKey).normalizedHex}`;
        } catch (error) {
            printError(jsonMode, error instanceof Error ? error.message : 'Invalid private key');
            return;
        }
    }
    ...
    saveConfig({
        ...
        ...(normalizedPrivateKey ? { private_key: normalizedPrivateKey } : {}),
