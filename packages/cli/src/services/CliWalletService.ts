...
export class CliWalletService {
    ...
    async getBalanceByAddress(address: string): Promise<AddressBalanceResult> {
        ...
        const balanceNano = BigInt(accountState.balance).toString();
        const balanceTon = formatUnits(balanceNano, 9);
    ...
    async getJettonsByAddress(address: string, limit: number = 20, offset: number = 0): Promise<JettonInfoResult[]> {
        const client = this.wallet.getClient();
        const response = await getJettonsFromClient(client, address, { pagination: { limit, offset } });
    ...
    async getJettonInfo(jettonAddress: string): Promise<JettonMetadataResult | null> {
        ...
        const jettonInfo = await kit.jettons.getJettonInfo(jettonAddress, network);
    ...
    async sendTon(toAddress: string, amountNano: string, comment?: string): Promise<TransferResult> {
        try {
            ...
            const txResult = await this.wallet.sendTransaction(tx);

            return {
                success: true,
                message: `Successfully sent ${amountNano} nanoTON to ${toAddress}`,
                normalizedHash: txResult.normalizedHash,
            };
        } catch (error) {
            return {
                success: false,
    ...
    async sendRawTransaction(request: {
        ...
        try {
            const txResult = await this.wallet.sendTransaction(request as TransactionRequest);
    ...
    async getNftsByAddress(address: string, limit: number = 20, offset: number = 0): Promise<NftInfoResult[]> {
        const client = this.wallet.getClient();
        const response = await getNftsFromClient(client, address, { pagination: { limit, offset } });
