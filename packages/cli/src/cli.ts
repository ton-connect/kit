...
import { balanceCommand, jettonBalanceCommand, balanceByAddressCommand } from './commands/balance.js';
import { transactionsCommand, transactionStatusCommand } from './commands/transactions.js';
import {
    jettonsCommand,
    knownJettonsCommand,
...
} from './commands/jettons.js';
import { dnsResolveCommand, dnsReverseCommand } from './commands/dns.js';
import { nftListCommand, nftListByAddressCommand, nftGetCommand, nftSendCommand } from './commands/nft.js';
...
function printHelp(): void {
    ...
    process.stderr.write('    balance address <addr>             Get TON balance for any address\n');
    ...
    process.stderr.write('    jettons by-address <addr>          List tokens for any address\n');
    process.stderr.write('    jettons info <jetton>              Get jetton metadata\n');
    process.stderr.write('    jettons wallet-address <j> <o>     Get jetton wallet address by owner\n');
    process.stderr.write('    transactions                       Recent transaction history\n');
    process.stderr.write('    transactions status <hash>         Get transaction status by normalized hash\n');
    ...
    process.stderr.write('    nft list-by-address <addr>         List NFTs for any address\n');
...
async function createWalletService(args: ParsedArgs): Promise<ServiceContext> {
    ...
    if (creds.mnemonic) {
        ...
        signer = await Signer.fromPrivateKey(parsePrivateKeyInput(creds.privateKey!).seed);
    }
...
async function main(): Promise<void> {
    ...
    try {
        switch (cmd) {
            ...
            case 'balance':
                if (sub === 'address' && rest[0]) {
                    await balanceByAddressCommand(service, rest[0], args.json);
                } else if (sub === 'jetton' && rest[0]) {
            ...
            case 'jettons':
                if (sub === 'by-address' && rest[0]) {
                    await jettonsByAddressCommand(service, rest[0], args.limit ?? 20, args.offset ?? 0, args.json);
                } else if (sub === 'info' && rest[0]) {
                    await jettonInfoCommand(service, rest[0], args.json);
                } else if (sub === 'wallet-address' && rest[0] && rest[1]) {
                    await jettonWalletAddressCommand(service, rest[0], rest[1], args.json);
                } else {
            ...
            case 'transactions':
                if (sub === 'status' && rest[0]) {
                    await transactionStatusCommand(service, rest[0], args.json);
                } else {
            ...
            case 'nft':
                if (sub === 'list') {
                ...
                } else if (sub === 'list-by-address' && rest[0]) {
                    await nftListByAddressCommand(service, rest[0], args.limit ?? 20, args.offset ?? 0, args.json);
                    ...
                    printError(
                        args.json,
                        'Invalid nft command.',
                        'Usage: ton nft list | ton nft list-by-address <addr> | ton nft get <addr> | ton nft send <nft> <to>',
                    );
                }
                break;
