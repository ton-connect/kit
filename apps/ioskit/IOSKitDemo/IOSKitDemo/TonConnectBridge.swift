...
class TonConnectBridge: UIViewController {
    ...
    func sendResponse(requestId: String, success: Bool, result: Any? = nil, error: String? = nil) {
        guard let webView = webView else { return }
...
extension TonConnectBridge: WKScriptMessageHandler {
    ...
    private func handleBridgeMessage(_ body: Any) {
        ...
        print("Bridge call: \(method) with args: \(args)")
        
        // Use async bridge method for enhanced functionality
        Task { @MainActor in
