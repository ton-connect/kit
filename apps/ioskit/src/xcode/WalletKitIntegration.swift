...
// MARK: - Enhanced TonConnect Bridge with WalletKit
extension TonConnectBridge {
    
    override func handleBridgeMessage(_ body: Any) {
        ...
        // Handle methods with WalletKit integration
        Task { @MainActor in
            await handleAsyncBridgeMethod(method: method, args: args, requestId: requestId)
        }
    }
    
    private func handleAsyncBridgeMethod(method: String, args: [Any], requestId: String) async {
        do {
            switch method {
            case "test":
                // Simple test method (keep as is)
                let response = [
